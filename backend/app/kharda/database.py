...
# DB is in the backend root, which is 3 levels up from here (app/kharda/database.py)
...
def get_latest_soiling_record(panel_id: int) -> Optional[Dict]:
    """Get the latest soiling record for a panel."""
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute("""
